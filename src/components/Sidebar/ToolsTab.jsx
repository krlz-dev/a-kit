...
export default function ToolsTab({
  ...
  if (selectedConnObj) {
    return (
      <LinkInspector conn={selectedConnObj} nodeMap={nodeMap} onDelete={onDeleteSelected} />
    );
  }
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        <div style={{ flex: 1, padding: "10px 12px", background: SURFACE, borderRadius: 10, textAlign: "center" }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: TEXT, fontFamily: "'JetBrains Mono', monospace" }}>{nodes.length}</div>
          <div style={{ fontSize: 9, color: TEXT_DIM, marginTop: 2 }}>Nodes</div>
        </div>
        <div style={{ flex: 1, padding: "10px 12px", background: SURFACE, borderRadius: 10, textAlign: "center" }}>
          <div style={{ fontSize: 18, fontWeight: 700, color: TEXT, fontFamily: "'JetBrains Mono', monospace" }}>{connections.length}</div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
        ...
        <button className="sidebar-btn" onClick={onUndo} disabled={undoStack.current.length === 0}>
          ...
          Undo
          <span style={{ marginLeft: "auto", fontSize: 10, color: TEXT_DIM, fontFamily: "'JetBrains Mono', monospace" }}>⌘Z</span>
        </button>
      ...
      <div style={{ marginTop: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          ...
          <span style={{ fontSize: 11, color: TEXT_MID, fontFamily: "'JetBrains Mono', monospace" }}>{speed.toFixed(1)}s</span>
        </div>
